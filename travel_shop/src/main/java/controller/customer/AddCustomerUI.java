...
public class AddCustomerUI extends BaseFrame {
    ...
    public AddCustomerUI() {
        ...
        gc.gridx=0; gc.gridy=y; leftCard.add(whiteLabel("customer_no(自動產生):"), gc);
        gc.gridx=1; leftCard.add(whiteLabel("(建立後自動產生)"), gc); y++;

        gc.gridx=0; gc.gridy=y; leftCard.add(whiteLabel("customer_name:"), gc);
    ...
    private void doSave() {
        try {
            ...
            Customer saved = service.register(c);
            AppSession.setCustomer(saved);
