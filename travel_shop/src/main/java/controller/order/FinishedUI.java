...
public class FinishedUI extends BaseFrame {
    ...
    public FinishedUI(String orderNo) {
        ...
        // ===== 標題 =====
        lblTitle = new JLabel("訂單完成", SwingConstants.CENTER);
        lblTitle.setFont(new Font("Microsoft JhengHei", Font.BOLD, 24));
        lblTitle.setForeground(GOLD);
        lblTitle.setBorder(new EmptyBorder(12,10,12,10));
        add(lblTitle, BorderLayout.BEFORE_FIRST_LINE);
        ...
        JButton btnPrint = cisGoldBtn("列印 CSV");
        JButton btnReorder = cisBlueBtn("重新下單");
        JButton btnBack = cisBlueBtn("回會員中心");

        bottom.add(btnPrint);
        bottom.add(btnReorder);
        bottom.add(btnBack);

        add(bottom, BorderLayout.SOUTH);

        btnPrint.addActionListener(e -> printCsv());
        btnReorder.addActionListener(e -> go(new OrderUI()));
    ...
    private void load(String orderNo) {
        ...
        if (o == null) { Message.error(this, "找不到訂單"); return; }

        lblTitle.setText("訂單完成 - order_no: " + o.getOrderNo() + " / final: NT$ " + o.getFinalAmount());
    ...
    private void printCsv() {
        ...
        fc.setSelectedFile(new File("finished_order.csv"));
